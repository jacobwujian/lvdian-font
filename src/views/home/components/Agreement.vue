...
export default {
  ...
  methods: {
    show(record) {
      ...
      this.agmPrice = Number(record.price) / 10 / 10 /10
    },
    // 当点击删除时 执行回调函数
    ...
    onUpload() {
      ...
      this.$api.device
        .protocolBuy({
          ...
          quantity: this.details.quantity * 10,
          price: this.agmPrice * 10 * 10 *10,
          amount: Number(this.agmPrice * 10 * 10 * 10 ) * Number(this.details.quantity* 10),
