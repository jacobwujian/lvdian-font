  ...
  <div class="home-container">
    ...
    <div class="price">
      ...
      <div>
        ...
        <div class="right">
          <a-button @click="openModel('recharge')">
            充值
          </a-button>
          <a-button @click="openModel('withdraw')">
            提现
          </a-button>
        </div>
      </div>
...
export default {
    ...
    return {
      ...
      columns1: [
        ...
        },
        {
          title: '金额',
          dataIndex: 'money',
          scopedSlots: { customRender: 'money', type: 'price' }
        },
        {
          title: '类型',
          dataIndex: 'type',
          scopedSlots: { customRender: 'type', type: 'select', data: dealType }
        },
        {
          title: '状态',
          dataIndex: 'status',
          scopedSlots: { customRender: 'status', type: 'select', data: payStatus }
        }
      ],
  ...
  methods: {
    ...
    fetch(params) {
      this.$api.trade.getCancelAfterVerification(params)
        .then(res => {
    ...
    fetch1(params) {
      this.$api.trade.rechargeWithdrawList(params)
          .then(res => {
            if(!res) res = {list: [], total: 0}
            this.data1 = res.list
            this.$refs.base1.fetchSuccess(res.total)
          })
          .finally(() => {
            this.$refs.base1.fetchFinally()
          })
    },
    openModel(type) {
